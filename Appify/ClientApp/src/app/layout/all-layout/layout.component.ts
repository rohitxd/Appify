...
export class LayoutComponent implements OnInit {
  ...
  constructor(public menuItems: MenuItems, private broadcast: Broadcast, private authentication: AuthenticationService) {
    this.broadcast.on<any>(this.authentication.tokenKey).subscribe(key => {
      var loginDetails = JSON.parse(key);
      this.userName = loginDetails.access_token.username;
      this.profilePicUrl = loginDetails.access_token.profilePicture;
    });
    var loginDetails = this.authentication.getToken();
    this.userName = loginDetails.access_token.username;
    this.profilePicUrl = loginDetails.access_token.profilePicture;
    var role = loginDetails.access_token.role;
    if (role === 1) {
      this.roleRoute = "sa";
    }
    if (role === 2) {
      this.roleRoute = "admin";
    }
